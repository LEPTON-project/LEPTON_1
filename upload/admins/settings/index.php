 ...
 * @version         $Id: index.php 1619 2012-01-11 14:05:03Z webbird $
 *
 */
...
function build_settings( &$admin, &$database )
{
    ...
    if ( $res_settings = $database->query( $sql ) )
    {
        while ( $row = $res_settings->fetchRow() )
        {
            ...
            $settings[ $row[ 'name' ] ]
				...
				: $row[ 'value' ];
			$tpl->set_var( array( strtoupper( $row[ 'name' ] ) => $row[ 'value' ] ) );
        }
    }
    ...
    $sql  = 'SELECT * FROM `' . TABLE_PREFIX . 'search` WHERE `extra` = \'\' ';
    if ( ( $res_search = $database->query( $sql ) ) && ( $res_search->numRows() > 0 ) )
    {
        while ( $row = $res_search->fetchRow() )
        {
            $search_settings[ $row[ 'name' ] ]
				= htmlspecialchars( ( $row[ 'value' ] ) );
            $tpl->set_var( array( 'SEARCH_'.strtoupper( $row[ 'name' ] ) => $row[ 'value' ] ) );
        }
    }
